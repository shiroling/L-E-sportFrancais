...
public class Jeu {
	...
	public static int getIdJeu(String nomJeu) {
		Connection connex = ConnexionBase.getConnectionBase();
		try {
			PreparedStatement st = connex.prepareStatement("SELECT id_Jeu from Jeu where Nom_jeu = ?");
			st.setString(1, nomJeu);
	...
	public static String[] getStringJeux() {
		List<Jeu> jeux = Jeu.getTousLesJeux();
